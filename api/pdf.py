...
async def consultar(dni: str, ruc: str):
    try:
...
# ✅ Nuevo endpoint: recibe el reporte directamente
@app.post("/api/pdf")
async def generar_pdf(reporte: str = Form(...)):
    try:
        data = json.loads(reporte)
        pdf_path = generar_reporte_pdf(data)
        return FileResponse(pdf_path, filename="Reporte_CarMatch.pdf", media_type="application/pdf")
    except Exception as e:
        return JSONResponse(content={"error": f"No se pudo crear el PDF: {str(e)}"})
